#pragma once
#include <Windows.h>
#include"_global_declarations.h"
//added a access of element by index function
		...
		namespace DataTypes
		{
			...
			**********************************************************************/
			...
			struct Reflexive
			{
				...
				//* Returns Tag Block Field Element at the specified index
				T* operator[](int i);
			private:
...
{	
	...
	char* MemPtr = SharedMapBase + TagBlockOffset;
	T** TagFields = MemPtr;
	return TagFields;
}
template<typename T>
T* Blam::Cache::DataTypes::Reflexive<T>::operator[](int i)
{
	return (T*)(SharedMapBase + TagBlockOffset + sizeof(T)*i);
}
template<typename T>
